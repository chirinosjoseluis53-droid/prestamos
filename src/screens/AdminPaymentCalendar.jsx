...
export default function AdminPaymentCalendar() {
  const { loans, getKycProfile, refreshLoans } = useData();
  ...
  const eventsByDate = useMemo(() => {
    ...
    loans
      ...
      .forEach((loan) => {
        ...
        const clientName = profile?.fullName || loan.clientEmail || 'Cliente';
  ...
  const monthStats = useMemo(() => {
    ...
    Object.entries(eventsByDate).forEach(([dateKey, events]) => {
      ...
      events.forEach((e) => {
        ...
        else if (e.status === 'overdue') overdue += 1;
        else pending += 1;
      });
    });
  ...
  return (
    <div className="admin-calendar animate-fade">
      {/* Stats Summary Bar */}
      <div className="grid-3col-stats" style={{ marginBottom: '24px' }}>
        <div className="premium-card" style={{ padding: '16px', display: 'flex', gap: '16px', alignItems: 'center' }}>
          <div style={{ padding: '10px', borderRadius: '10px', backgroundColor: 'rgba(15,164,108,0.1)', color: 'var(--primary)' }}>
            <Clock size={20} />
          </div>
          <div>
            <div style={{ fontSize: '20px', fontWeight: '800' }}>{monthStats.pending}</div>
            <span style={{ fontSize: '12px', color: 'var(--text-secondary)' }}>Pendientes del mes</span>
          </div>
        </div>

        <div className="premium-card" style={{ padding: '16px', display: 'flex', gap: '16px', alignItems: 'center' }}>
          <div style={{ padding: '10px', borderRadius: '10px', backgroundColor: 'rgba(239,68,68,0.1)', color: 'var(--danger)' }}>
            <AlertCircle size={20} />
          </div>
          <div>
            <div style={{ fontSize: '20px', fontWeight: '800', color: 'var(--danger)' }}>{monthStats.overdue}</div>
            <span style={{ fontSize: '12px', color: 'var(--text-secondary)' }}>Vencidas del mes</span>
          </div>
        </div>

        <div className="premium-card" style={{ padding: '16px', display: 'flex', gap: '16px', alignItems: 'center' }}>
          <div style={{ padding: '10px', borderRadius: '10px', backgroundColor: 'rgba(16,185,129,0.1)', color: 'var(--success)' }}>
            <CheckCircle size={20} />
          </div>
          <div>
            <div style={{ fontSize: '20px', fontWeight: '800', color: 'var(--success)' }}>{monthStats.paid}</div>
            <span style={{ fontSize: '12px', color: 'var(--text-secondary)' }}>Pagadas del mes</span>
          </div>
        </div>
      </div>
        ...
        <div className="premium-card" style={{ padding: '24px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              <button className="btn btn-secondary" style={{ padding: '6px' }} onClick={() => changeMonth(-1)}>
                <ChevronLeft size={16} />
              </button>
            ...
            <button className="btn btn-secondary" style={{ padding: '6px 12px', fontSize: '13px' }} onClick={goToday}>
              Hoy
            </button>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: '6px' }}>
            {cells.map((day, idx) => {
              ...
              const dotColor = hasOverdue
                ? 'var(--danger)'
                : dayEvents.some(e => e.status === 'submitted')
                  ? 'var(--warning)'
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    borderRadius: '10px',
                    border: '1px solid',
                    borderColor: isSelected ? 'var(--primary)' : isToday ? 'rgba(15,164,108,0.3)' : 'var(--border)',
                    backgroundColor: isSelected ? 'rgba(15,164,108,0.1)' : 'var(--surface-light)',
                    display: 'flex',
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', flex: 1, overflowY: 'auto' }}>
            {selectedEvents.length === 0 ? (
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '150px', gap: '10px' }}>
                ...
                <span style={{ fontSize: '13px', color: 'var(--text-secondary)' }}>Sin cuotas programadas para este día</span>
              </div>
            ) : (
              selectedEvents.map(event => (
                ...
                >
                  <div>
                    <h5 style={{ margin: 0, fontSize: '14px', fontWeight: '700' }}>{event.clientName}</h5>
                    <span style={{ fontSize: '12px', color: 'var(--text-secondary)' }}>
                      Cuota #{event.number} · ${event.amount.toFixed(2)}
                    </span>
                  </div>
                  <span style={{ 
                    fontSize: '11px', 
                    ...
                    border: `1px solid ${STATUS_COLORS[event.status]}33`
                  }}>
